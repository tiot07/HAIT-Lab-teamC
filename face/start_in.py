        ...
        for (pos_x, pos_y, w, h) in face_list:
            ...
            if i>10:
              ...
              try:
                ...
                if (df.tail(1).iloc[0][0]==names[np.argmax(results[0])] and df.tail(1).iloc[0][2]=='in'):
                  pass
                else:
                  df=df.append([[names[np.argmax(results[0])],t,"in","{}-{}.jpg".format(name,t)]])
              except FileNotFoundError:
                df=pd.DataFrame(data=[[names[np.argmax(results[0])],t,"in","{}-{}.jpg".format(name,t)]])
              
              #slack(name)
              df.to_csv('../face_log.csv', header=False, index=False)
              cv2.imwrite("../static/images/{}-{}.jpg".format(name,dt.now().strftime('%Y%m%d%H%M%S')), img)
              i=0
